...
import { Plus, Trash2, Image, GripVertical, Star, Shuffle, Clock, Monitor, Upload, ChevronDown, ChevronRight, Settings2 } from "lucide-react";
...
import { useSignage, type SignagePlaylist, type SignageSlide } from "@/hooks/useSignage";
import { useDisplays } from "@/hooks/useDisplays";
import { toast } from "sonner";

export default function GymSignage() {
  const { selectedGymId } = useOutletContext<{ selectedGymId: string | null }>();
  const { playlists, isLoading, createPlaylist, updatePlaylist, deletePlaylist, fetchSlides, addSlide, updateSlide, deleteSlide, uploadSlideImage, fetchAssignments, assignPlaylist, removeAssignment, assignToAllDisplays } = useSignage("gym", selectedGymId);
  const { displays } = useDisplays("gym", selectedGymId);
  const [createOpen, setCreateOpen] = useState(false);
  const [newName, setNewName] = useState("");
  const [expandedPlaylist, setExpandedPlaylist] = useState<string | null>(null);
  const [slides, setSlides] = useState<Record<string, SignageSlide[]>>({});
  const [assignments, setAssignments] = useState<Record<string, string>>({}); // displayId -> playlistId
  ...
  const handleFileUpload = async (playlistId: string, files: FileList | null) => {
    ...
    for (const file of Array.from(files)) {
      ...
      if (url) {
        await addSlide(playlistId, url, nextOrder);
        nextOrder++;
      }
    }
  ...
  const handleSlideDuration = async (slideId: string, playlistId: string, value: string) => {
    const num = parseInt(value);
    await updateSlide(slideId, { duration_seconds: isNaN(num) ? null : num });
    const updatedSlides = await fetchSlides(playlistId);
    setSlides(prev => ({ ...prev, [playlistId]: updatedSlides }));
  };
          ...
          <DialogContent>
            ...
            <div className="space-y-4">
              <div>
                ...
                <Input placeholder="e.g. Default Gym Signage" value={newName} onChange={e => setNewName(e.target.value)} />
              </div>
      ...
      {isLoading ? (
        ...
        <Card>
          <CardContent className="flex flex-col items-center justify-center py-12">
            ...
            <p className="text-sm text-muted-foreground">Create a playlist and upload images to show on your displays</p>
          </CardContent>
        </Card>
      ) : (
        <div className="space-y-4">
                ...
                <CollapsibleContent>
                  <CardContent className="space-y-4 pt-0">
                    ...
                    <div className="flex gap-2">
                      <input ref={fileInputRef} type="file" multiple accept="image/*" className="hidden" onChange={e => handleFileUpload(playlist.id, e.target.files)} />
                        ...
                        <div key={slide.id} className="relative group rounded-lg overflow-hidden border border-border">
                          ...
                          <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
                            <Input
                              ...
                              className="w-20 h-7 text-xs bg-black/50 border-white/20 text-white"
                              defaultValue={slide.duration_seconds || ""}
                              onBlur={e => handleSlideDuration(slide.id, playlist.id, e.target.value)}
                            />
                          ...
                          {slide.caption && (
                            <div className="absolute bottom-0 left-0 right-0 bg-black/60 px-2 py-1">
                              <p className="text-xs text-white truncate">{slide.caption}</p>
                            </div>
                          )}
                        </div>
                      ))}
                    ...
                    <div className="flex justify-end pt-2 border-t border-border">
                      <Button variant="ghost" className="text-destructive gap-1.5" onClick={() => { if (confirm("Delete this playlist and all its slides?")) deletePlaylist(playlist.id); }}>
